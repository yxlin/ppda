...
__global__ void squareSum_kernel(unsigned int* n, double *g_idata, double *g_tmp, double *g_odata) {
  __shared__ double sdata[256];
...
void sqsum_entry(double *RT0, int *nRT0, int *nth, bool *debug, double *out) {
  ...
  double *d_RT0;
  double *d_pssum, *h_pssum, *d_squareBucket, sum2 = 0;
  ...
  *h_nRT0 = (unsigned int)*nRT0;
  ...
  CHECK(cudaMalloc((void**) &d_RT0,  *nRT0 * sizeof(double)));
  CHECK(cudaMalloc((void**) &d_nRT0,     1 * sizeof(unsigned int)));
  CHECK(cudaMalloc((void**) &d_squareBucket,  *h_nRT0 * sizeof(double)));
  ...
  squareSum_kernel<<<nBlk, nThread>>>(d_nRT0, d_RT0, d_squareBucket, d_pssum);
  ...
  *out = sum2 ;
  ...
  cudaFree(d_RT0);  cudaFree(d_nRT0);
  cudaFree(d_pssum); cudaFree(d_squareBucket);
}
...
void sd_entry(double *RT0, int *nRT0, int *nth, bool *debug, double *out) {
  ...
  *out = std::sqrt( (sum2 - (sum1*sum1) / *h_nRT0) / (*h_nRT0 - 1) );
  ...
  cudaFree(d_pssum); cudaFree(d_squareBucket); cudaFree(d_mean);
}
