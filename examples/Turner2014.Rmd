...
values and then fit this data set with our parallel PDA.  For 
each proposal, we simulated the model 16384 times (2^14). This example shows 
that the parallel PDA is able to replicate Turner and Sederberg's (2014) 
finding. 
...
system.time(samples5 <- run.dmc(samples.dmc(nmc = 500, samples = samples4),
                                cores = 1, report = 100, force = 4))
check.recovery.dmc(samples5, pvec) ## with 2^14 model simulations
##                    A     B mean_v.true mean_v.false   t0
## True            0.25  0.35        1.00         0.25 0.20
## 2.5% Estimate   0.01  0.03       -5.49       -11.07 0.11
## 50% Estimate    0.14  0.09       -0.47        -1.74 0.25
## 97.5% Estimate  0.59  0.52        1.32         0.48 0.29
## Median-True    -0.11 -0.26       -1.47        -1.99 0.05

## Using 2^20 model simulations with only 500 iterations 400 burnin's ----
## We can quickly acheive good estimates. So the price we pay to get more 
## precise likelihood is repaid by achieving reliable estimation with
## less MC iterations.
samples <- samples.dmc(nmc = 400, p.prior, mdi0)
system.time(samples0 <- run.dmc(samples, report = 50, cores = 1, force = 4,
                                p.migrate = .05))
