...
__global__ void sumur_kernel(unsigned int *n, T *in, T *out) {
    T *cache = SharedMemory<T>();
    unsigned int tid = threadIdx.x;
    unsigned int i   = blockIdx.x * (blockSize * 2) + threadIdx.x;
    unsigned int gridSize = blockSize * 2 * gridDim.x;
    cache[tid] = 0;
    while (i < *n) { cache[tid] = in[i] + in[i + blockSize]; i += gridSize; }
    __syncthreads();
...
void sumur_entry(double *x, int *nx, bool *debug, double *out) {
    ...
    *h_nx = (unsigned int)*nx;
    ...
    sumur_kernel<float, 256><<< nBlk, nThread, smemSize >>>(d_nx, d_x, d_out);
    CHECK(cudaMemcpy(h_out, d_out, blockSize,   cudaMemcpyDeviceToHost));

    for (unsigned int i=0; i<nBlk; i++) { out[0] += (double)h_out[i]; }
